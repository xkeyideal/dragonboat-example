...
const (
	deploymentId = 34723465

	rafttimeout = 3 * time.Second // raft节点操作超时时间

	moveToTimeout = 4 * time.Second // moveTo操作的超时时间
...
func buildRaftConfig(nodeId, clusterId uint64) config.Config {
	rc := config.Config{
		...
		// 在Raft中，选举之间的实际间隔被随机分配在ElectionRTT和2 * ElectionRTT之间。
		// 例如，假设NodeHostConfig.RTTMillisecond为100毫秒，要将选举间隔设置为1秒，则应该将ElectionRTT设置为10。
		// 启用CheckQuorum后，ElectionRTT还将定义检查领导者定额的时间间隔。

		// 这个值是个比例,具体的RTT时间大小是RTTMillisecond*ElectionRTT,当需要选举主节点时,
		// 各个节点的随机间隔在ElectionRTT和2 * ElectionRTT,当CheckQuorum为true,主也会每隔这个时间检查下从机数据是否符合法定人数
		ElectionRTT: 20,

		// HeartbeatRTT是两次心跳之间的消息RTT数。 消息RTT由NodeHostConfig.RTTMillisecond定义。
		// Raft论文建议心跳间隔应接近节点之间的平均RTT。
		...
		// 生成快照后，可以压缩新快照覆盖的Raft日志条目。这涉及两个步骤，冗余日志条目首先被标记为已删除，
		// 然后在稍后发布 LogDB 压缩时将其从基础存储中物理删除。有关在生成快照后实际删除和压缩哪些日志条目的详细信息，
		// 请参见CompactionOverhead,通过将SnapshotEntries字段设置为0禁用自动快照后，用户仍然可以使用NodeHost的
		// RequestSnapshot或SyncRequestSnapshot方法手动请求快照。
		SnapshotEntries: 25 * 10000 * 10,

		// CompactionOverhead定义每次Raft日志压缩后要保留的最新条目数。假设当前的日志为10000,开始创建快照,
		// 那么快照创建完成后,<=10000的日志都会被清理,如果想获得9000这样的日志,那么就得先完全加载快照,再从快照中读取,
		// 如果设置了CompactionOverhead为3000,那么就算创建了快照,我们仍然能获得10000-7000之间的日志记录,
		// 只有小于7000的,才需要重新加载日志获取
		CompactionOverhead: 25 * 10000,

		//确定是否使用ChangeID的顺序强制执行Raft成员资格更改。
		OrderedConfigChange: true,

		// MaxInMemLogSize是允许在每个Raft节点上的Raft日志存储在内存中的目标大小（以字节为单位）。
		// 内存中的筏日志是尚未应用的日志。
		...
		// IsObserver指示当前节点是否是Observer节点,(观察者节点通常用于允许新节点加入群集并追赶其他日志，而不会影响可用性。
		// 还可以引入额外的观察者节点来满足只读请求，而不会影响系统的写吞吐量)
		IsObserver: false,
...
func buildNodeHostConfigByGossip(raftDir string, nodeId uint64, gossipPort uint16, hostIP string, raftAddr string,
	...
	return config.NodeHostConfig{
		...
		// WALDir是用于存储所有Raft日志的WAL的目录,这仅用于存储Raft日志的WAL，它的大小通常很小，每个NodeHost的64GB通常绰绰有余。
		...
		Gossip: config.GossipConfig{
			BindAddress: fmt.Sprintf("%s:%d", hostIP, gossipPort),
			Seed:        gossipSeeds,
		},
		// ListenAddress是Raft RPC模块用于侦听Raft消息和快照的IP：端口地址。 如果未设置ListenAddress字段，则Raft RPC模块将使用RaftAddress。
		...
		// 当配置了TLS时,需要指定CertFile的地址 CertFile string
		// 当配置了TLS时,需要指定KeyFile的地址 KeyFile string
		// MaxReceiveQueueSize是每个接收队列的最大大小（以字节为单位）。
		// 一旦达到最大大小，将删除更多复制消息以限制内存使用。 设置为0时，表示队列大小不受限制。
		// 暂时先设置为128M
...
func buildNodeHostConfig(raftDir string, raftAddr string,
	metrics bool, re *raftEvent, se *systemEvent) config.NodeHostConfig {
	return config.NodeHostConfig{
		// DeploymentID用于确定两个NodeHost实例是否属于同一部署，并因此允许彼此通信。
		// 通过将上下文消息发送到不相关的Raft节点，这有助于防止意外配置错误的NodeHost实例导致数据损坏错误。
		// 对于特定的基于Dragonboat的应用程序，可以在所有生产NodeHost实例上将DeploymentID设置为相同的uint64值，
