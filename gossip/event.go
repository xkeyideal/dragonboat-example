...
func (ai *aliveInstance) updateInstance(meta *Meta, alive bool) {
	...
	ai.serverInstances[meta.ServerGrpcAddr] = alive
	ai.mu.Unlock()
}
...
func (ai *aliveInstance) getserverInstances() map[string]bool {
	ai.mu.RLock()
	defer ai.mu.RUnlock()

	return ai.serverInstances
}
